...
namespace KERBALISM
{
		...
		static RemoteTech()
		{
			foreach (var a in AssemblyLoader.loadedAssemblies)
			{
				if (a.name == "RemoteTech")
				{
					...
					if(Enabled)
						API.SetAntennaInfoFactory(new RT_AntennaInfoFactory());
	...
	class RT_AntennaInfoFactory : IAntennaInfoFactory
	{
		public IAntennaInfo Create(Vessel v, bool powered, bool storm)
		{
			RemoteTech.SetPoweredDown(v.id, !powered);
			RemoteTech.SetCommsBlackout(v.id, storm);
			return new AntennaInfoRT(v);
		}
	}
