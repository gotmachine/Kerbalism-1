...
namespace KERBALISM
{
	...
	public sealed class SolarPanelFixer : PartModule
	{
		...
		bool analyticalSunlight;

		public enum SupportedPanelType
		{
			Undefined = 0,
			ModuleDeployableSolarPanel,	// stock panel
			ModuleCurvedSolarPanel,     // NFS curved panel, custom implementation
			SSTUSolarPanelStatic,       // SSTU fixed panel, custom implementation
			SSTUSolarPanelDeployable,   // SSTU deployable panel, custom implementation
			SSTUModularPart             // SSTU part with integrated solar panels, 999% custom implemention
		}
		...
		public override void OnStart(StartState state)
		{
			...
			Type panelEnumType = typeof(SupportedPanelType);
			...
			foreach (PartModule pm in part.Modules)
			{
				...
				else if (Enum.IsDefined(panelEnumType, pm.moduleName)) // other supported modules
				{
					SupportedPanelType solarPanelType = (SupportedPanelType)Enum.Parse(panelEnumType, pm.moduleName);

					switch (solarPanelType)
					{
						case SupportedPanelType.ModuleCurvedSolarPanel: solarPanel = new NFSCurvedPanel(); break;
						case SupportedPanelType.SSTUSolarPanelStatic: break;
						case SupportedPanelType.SSTUSolarPanelDeployable: break;
						case SupportedPanelType.SSTUModularPart: break;
					}
				}
		...
		public void FixedUpdate()
		{
			...
			{
				if (!analyticalSunlight && vessel.Landed)
				{
					persistentFactor = GetAnalyticalCosineFactorLanded(info.sun_dir);
				}
				analyticalSunlight = true;
			}
			else
			{
				analyticalSunlight = false;
			}
